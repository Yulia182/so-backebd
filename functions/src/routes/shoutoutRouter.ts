...
shoutoutRouter.get("/shoutouts", async (req, res) => {
  // best practice: when filter in API, use query string params
  // when getting one specific piece of data, use path param
  const toQueryStrinParam: string | undefined = req.query["to-name"] as string;
  const me: string | undefined = req.query.me as string;
  const query: any = {};
  if (toQueryStrinParam) {
    query.to = toQueryStrinParam;
  } else if (me) {
    query.$or = [{ toQueryStrinParam: me }, { from: me }];
  }
  try {
...
// can't use shoutouts/top-five, will conflict with previous and
// interprete it like id
